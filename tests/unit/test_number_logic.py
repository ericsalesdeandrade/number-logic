...
def test_read_input_file(number_logic):
...
def test_read_input_file_exception(number_logic):
...
def test_convert_str_int(number_logic):
    int_list = [10, 20, 30]
...
def test_convert_str_int_exception(number_logic):
...
def test_check_sum(number_logic):
...
def test_multiply_values(number_logic):
...
def test_multiply_values_exception(number_logic):
    with pytest.raises(ValueError):
